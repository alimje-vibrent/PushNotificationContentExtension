...
//  VHFeature-iOS
//
//  Created by Ashish Limje on 15/06/22.
//
...
class NotificationViewControllerParent: UIViewController, UNNotificationContentExtension, UITextViewDelegate {
    ...
    func didReceive(_ notification: UNNotification) {
        ...
        if notification.request.content.categoryIdentifier == "NOTIFICATION_CATEGORY"{
            if let aps = notification.request.content.userInfo[APNSPayloadKey.aps] as? [String: Any], let alert = aps[APNSPayloadKey.alert] as? [String: Any], let body = alert[APNSPayloadKey.body] as? String, let title = alert[APNSPayloadKey.title] as? String {
